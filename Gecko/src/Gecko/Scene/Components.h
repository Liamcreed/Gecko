...
namespace Gecko
{
    ...
    struct MeshRendererComponent
    {
        ...
        MeshRendererComponent(std::string  path)
            :Path(path)  { model->LoadFromFile(Path); }
        void LoadFromFile(std::string path){Path = path; model->LoadFromFile(Path);}

        Ref<Model> model = CreateRef<Model>();
    };
    enum LightType { Sun = 0, PointLight = 1, SpotLight = 2};
